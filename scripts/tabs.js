(function() {
  ...
  let archeTabCounter = 0;
  // Count the number of panels use to generate unique ids
  // Could this be a static method
  let archePanelCounter = 0;

  // Define a template for the arche tabs container
  ...
  // Define the archeTabs class it extends HTMLElement 
  // and will be used to create a new HTML tag
  class archeTabs extends HTMLElement {
    constructor() {
    ...
    connectedCallback() {
      ...
      Promise.all([
        customElements.whenDefined('arche-tab'),
        customElements.whenDefined('arche-panel'),
      ])
    ...
    _linkPanels() {
      ...
      tabs.forEach(tab => {
        ...
        if (panel.tagName.toLowerCase() !== 'arche-panel') {
          console.error(`Tab #${tab.id} is not a` +
            `sibling of a <arche-panel>`);
          return;
        }
    ...
    _allPanels() {
      return Array.from(this.querySelectorAll('arche-panel'));
    }

    // Get all the tabs
    _allTabs() {
      return Array.from(this.querySelectorAll('arche-tab'));
    }
    ...
    customElements.define('arche-tabs', archeTabs);
    
    // -----------------------------------------------------
    // make a class for a how tab
    class archeTab extends HTMLElement {
      ...
      connectedCallback() {
        ...
        if (!this.id)
          this.id = `arche-tab-generated-${archeTabCounter++}`;
    ...
    // Define the new custom element 
    customElements.define('arche-tab', archeTab);

    // ----------------------------------------------------
    
    // Define the arche panel Component 

    class archePanel extends HTMLElement {
      constructor() {
        super(); // must call super!
      }
      ...
      connectedCallback() {
        ...
        if (!this.id)
          this.id = `arche-panel-generated-${archePanelCounter++}`;
      }
    }

    // Define the custom element 
    customElements.define('arche-panel', archePanel);
  })();
